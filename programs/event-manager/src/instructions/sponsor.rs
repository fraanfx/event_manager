...
pub struct Sponsor<'info> {
    ...
    #[account(
        mut, 
        seeds = [
            Event::SEED_EVENT_MINT.as_ref(), // "event:mint" seed
    ...
    //account to accept mint
    // payer accepted mint ATA (user wallet that contains the accepted mints)
    #[account(
        mut, 
    ...
    //Mint new tokens for the sponsorship (event)
    #[account(
        init_if_needed, // create account id doesn't exist
        payer = authority,
