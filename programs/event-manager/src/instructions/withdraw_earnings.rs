...
pub struct WithdrawEarnings<'info> {
    ...
    #[account(
        ...
        seeds = [
            Event::SEED_EVENT.as_ref(), // "event" seed
            event.authority.key().as_ref() // event public key
        ],
    ...
    #[account(mut)]
    // account receivieing funds -> example:
    ...
    #[account(
        mut,
        seeds = [
            Event::SEED_GAIN_VAULT.as_ref(), // "gain vault" seed
        ],
...
pub fn handle(  ctx: Context<WithdrawEarnings> ) -> Result<()> {
    // total event mint tokens sold ( total in treasury)
    let total_tokens = ctx.accounts.event.sponsors; // withdraw 1 billion event
    ...
    // calculate total earning amount basde on share %
    ...
    burn(
        CpiContext::new(
            ...
            Burn { 
                ...
                authority: ctx.accounts.authority.to_account_info(), // burn authority ( the user owner of the account)
            }
        ),
    ...
    let seeds = [
        Event::SEED_GAIN_VAULT.as_ref(), // "event" seed
